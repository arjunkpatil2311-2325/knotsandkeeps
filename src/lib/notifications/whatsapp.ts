...
export async function sendOrderNotificationWhatsApp(orderNumber: string, event: NotificationEvent) {
  ...
  switch (event) {
    ...
    case 'payment_verified':
      ...
      messageBody = `🎉 Your Knots & Keeps order ${order.order_number} is confirmed!\n\nProduct:\n${itemsList}\n\nTotal:\n₹${order.total_amount.toFixed(2)}\n\nAmount paid:\n₹${order.amount_paid.toFixed(2)}\n\nRemaining:\n₹${order.amount_remaining.toFixed(2)}\n\nDelivery:\n${order.delivery_method} Delivery\n\nEstimated delivery:\n${order.delivery_method === 'fast' ? '3-5 days' : '7-9 days'}\n\nThank you for shopping with Knots & Keeps! 💝`
      break
    case 'packed':
      messageBody = `📦 Your Knots & Keeps order ${order.order_number} has been packed and is getting ready to ship!`
      break
    case 'shipped':
      messageBody = `🚚 Your Knots & Keeps order ${order.order_number} has been shipped!\n\nCourier:\n${order.courier_name || 'N/A'}\n\nTracking number:\n${order.tracking_number || 'N/A'}\n\nTracking:\n${order.tracking_url || 'N/A'}`
      break
    case 'out_for_delivery':
      messageBody = `🏠 Your Knots & Keeps order ${order.order_number} is out for delivery today!`
      break
    case 'delivered':
      messageBody = `💝 Your Knots & Keeps order ${order.order_number} has been delivered!\n\nThank you for shopping with Knots & Keeps!`
      break
  }
